...
def _preload_patched_sapien_urdf_loader():
    """Work around a broken local SAPIEN 3.0.0b1 install without editing site-packages."""
    try:
        import sapien.wrapper  # noqa: F401
    except Exception:
        return
    ...
    source_path = site_packages / "sapien" / "wrapper" / "urdf_loader.py"
    if not source_path.is_file():
        return

    source = source_path.read_text(encoding="utf-8")
    broken = 'encoding="utf-8" as f:'
    fixed = 'encoding="utf-8") as f:'
    ...
    exec(compile(source.replace(broken, fixed), str(source_path), "exec"), module.__dict__)
    print(f"[chw] preloaded patched {module_name} from {source_path}")


def _append_episode_record(task_name, args, episode_id, seed, success):
    log_path = os.environ.get("ROBOTWIN_EPISODE_LOG", "").strip()
    if not log_path:
        return
    path = Path(log_path)
...
_preload_patched_sapien_urdf_loader()
...
def eval_policy(task_name,
    ...
    expert_check = os.environ.get("ROBOTWIN_EXPERT_CHECK", "1").strip().lower() not in {"0", "false", "no"}
    TASK_ENV.suc = 0
    TASK_ENV.test_num = 0
    ...
    max_seed_attempts = int(os.environ.get("ROBOTWIN_MAX_SEED_ATTEMPTS", max(test_num * 50, 50)))

    args["eval_mode"] = True

    while succ_seed < test_num:
        if now_seed - st_seed >= max_seed_attempts:
            raise RuntimeError(
                f"Exceeded ROBOTWIN_MAX_SEED_ATTEMPTS={max_seed_attempts} while collecting "
                f"{test_num} valid episodes for {task_name}."
            )
        ...
        _append_episode_record(task_name, args, now_id, now_seed, succ)
        now_id += 1
